...
{ 
	...
	if (gpGlobals->deathmatch)
	{
		if (debug_engine) { fp=fopen("spirit\\bot\\debug.txt", "a"); fprintf(fp, "ClientConnect: pent=%x name=%s\n",pEntity,pszName); fclose(fp); }
...
void ClientDisconnect( edict_t *pEntity )
{
	...
	if (gpGlobals->deathmatch) {
		if (debug_engine) { fp=fopen("spirit\\bot\\debug.txt", "a"); fprintf(fp, "ClientDisconnect: %x\n",pEntity); fclose(fp); }
...
{
   if (debug_engine) { fp=fopen("spirit\\bot\\debug.txt", "a"); fprintf(fp, "ClientKill: %x\n",pEntity); fclose(fp); }
...
{
	if (debug_engine) { fp=fopen("spirit\\bot\\debug.txt", "a"); fprintf(fp, "ClientPutInServer: %x\n",pEntity); fclose(fp); }
	
	int index = 0;
	while ((index < 32) && (clients[index] != NULL)) index++;
	if (index < 32) clients[index] = pEntity;
	else {
		FILE *dfp=fopen( "spirit/bot/crashlog.txt", "a" ); 
...
{
   if (debug_engine) { fp=fopen("spirit\\bot\\debug.txt", "a"); fprintf(fp, "ClientUserInfoChanged: pEntity=%x infobuffer=%s\n", pEntity, infobuffer); fclose(fp); }
...
{
	if (debug_engine) { fp=fopen("spirit\\bot\\debug.txt", "a"); fprintf(fp, "ServerDeactivate\n"); fclose(fp); }
...
{
   if (debug_engine) { fp=fopen("spirit\\bot\\debug.txt", "a"); fprintf(fp, "PlayerCustomization: %x\n",pEntity); fclose(fp); }
...
{
	if (debug_engine) { fp=fopen("spirit\\bot\\debug.txt", "a"); fprintf(fp, "Sys_Error: %s\n", error_string); fclose(fp); }
...
{
   if (debug_engine) { fp=fopen("spirit\\bot\\debug.txt", "a"); fprintf(fp, "InconsistentFile: %x filename=%s\n",player,filename); fclose(fp); }
